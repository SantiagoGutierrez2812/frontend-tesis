...
export default function Materialcreation() {
  ...
  const [nextId, setNextId] = useState(1); // autoincremental local
  ...
  const confirmSave = async () => {
    ...
    try {
      if (editingId) {
        ...
        setMaterials(prev =>
          prev.map(mat =>
            mat.id === editingId ? { id: editingId, nombre, tamaño, precio: numericPrecio, descripcion } : mat
          )
        );
        ...
        setMaterials(prev => [...prev, { id: productId, nombre, tamaño, precio: numericPrecio, descripcion }]);
      ...
      const errorMessage = err instanceof Error ? err.message : "Error desconocido al guardar el material ❌";
  ...
  const ConfirmationModal = () => (
    <div className="modal_1" onClick={() => setShowConfirmationModal(false)}>
      <div className="modal-content_1">
        ...
        <p>¿Guardar el material <strong>{newMaterial.nombre}</strong>?</p>
        <div className="modal-actions">
          <button className="btn-cancel" onClick={() => setShowConfirmationModal(false)}>Cancelar</button>
          <button className="btn-save" onClick={confirmSave}>Aceptar</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="dashboard-container">
      <TopControl title="📦 Panel de Gestión de Materiales" />
      <ToastContainer />

      <div className="control-bar">
        <input type="text" placeholder="🔍 Buscar material..." className="search-input" />
        <button className="btn-primary" onClick={() => { setShowModal(true); setEditingId(null); }}>+ Crear Material</button>
      </div>
          ...
          <tbody>
            {materials.length ? materials.map(mat => (
              <tr key={mat.id}>
                ...
                <td>{mat.precio.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 })}</td>
      ...
      {showModal && (
        <div className="modal_1" onClick={handleModalClick}>
          <div className="modal-content_1">
            ...
            <input placeholder="Tamaño" value={newMaterial.tamaño} onChange={e => setNewMaterial({ ...newMaterial, tamaño: e.target.value })} />

            <label>Descripción</label>
            <input placeholder="Descripción" value={newMaterial.descripcion} onChange={e => setNewMaterial({ ...newMaterial, descripcion: e.target.value })} />

            <label>Precio Unitario ($)</label>
            <input type="number" placeholder="Precio" value={newMaterial.precio} onChange={e => setNewMaterial({ ...newMaterial, precio: e.target.value })} />

            <div className="modal-actions">
              <button className="btn-cancel" onClick={() => setShowModal(false)}>Cancelar</button>
              <button className="btn-save" onClick={handleSave}>{editingId ? "Actualizar" : "Guardar"}</button>
            </div>
          </div>
        </div>
      )}
